...
function init() {
    ...
    // An AmbientLight brightens the whole scene, so the model isn't completely black.
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.6); // soft white light
    sceneSpace.add(ambientLight);

    // A DirectionalLight acts like a sun, casting shadows and giving highlights.
    const directionalLight = new THREE.DirectionalLight(0xffffff, 1.5);
    directionalLight.position.set(500, 500, 500); // Position the light source
    sceneSpace.add(directionalLight);

    // <<< ADDED ENVIRONMENT MAP FOR REFLECTIONS AND COLOR >>>
    // This gives the model realistic reflections from its surroundings.
    const cubeTextureLoader = new THREE.CubeTextureLoader();
    // IMPORTANT: Replace these paths with your actual cubemap image files.
    const environmentMapTexture = cubeTextureLoader.load([
        './content/cubemap/px.png', // Right
        './content/cubemap/nx.png', // Left
        './content/cubemap/py.png', // Top
        './content/cubemap/ny.png', // Bottom
        './content/cubemap/pz.png', // Front
        './content/cubemap/nz.png', // Back
    ]);

    // Set the scene's background and environment lighting from the map.
    sceneSpace.background = environmentMapTexture;
    sceneSpace.environment = environmentMapTexture;
    ...
    falconLoader.load('./content/models/MilleniumFalcon/model2.dae', function (collada) {
        falconPivot = new THREE.Group();
        falcon = collada.scene;

        // Set the scale of the model. Here, we make it 5 times bigger on all axes.
        falcon.scale.set(5, 5, 5); 
        ...
        falcon.traverse(function (child) {
            if (child.isMesh) { // Check if it's a mesh before setting shadow properties
...
function onMouseMove(event) {
    ...
    // Horizontal drag → rotate around Y (left/right)
    camera.rotateOnWorldAxis(vectorY, -deltaX * mouseRotationSpeed);

    // Vertical drag → rotate around X (up/down)
    camera.rotateOnAxis(vectorX, -deltaY * mouseRotationSpeed);
...
function startGUI() {
    ...
    gui.add({mouseRotationSpeed}, 'mouseRotationSpeed')
   .min(0.001).max(0.05).step(0.001)
   .name('Mouse Rotation Speed')
   .onChange(value => mouseRotationSpeed = value);
...
function startGUI() {
    ...
    shipSoundFolder.add(params, 'RestartShipMusic').name('Restart music');
                         ...
                         add mouse interaciton for rotation insted of  up,down,left,right keys give whole code